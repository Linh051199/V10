...
import { selectedItemsAtom } from "./ser-mst-service";
import { Search_Ser_MST_Service } from "@/packages/types/master/Ser_MST_Service";
import { usePermissions } from "@/packages/contexts/permission";

interface HeaderPartProps {
  onAddNew: () => void;
  searchCondition: Partial<Search_Ser_MST_Service>;
  handleRefetch: () => void;
}
export const HeaderPart = ({
  ...
  const onDownloadTemplate = async () => {
    const respone = await api.Ser_MST_Service_Export_Template();
  ...
  const handleUploadFiles = async (files: File[]) => {
    const respone = await api.Ser_MST_Service_Upload(files[0]);
    if (respone.isSuccess) {
      closeButton();
      handleRefetch();
      ...
      // notify(t(respone.Data._strErrCode), {
      //   position: {
      //     top: 0,
      //   },
      //   direction: "down-push",
  ...
  const handleExportExcel = async (selectedOnly: boolean) => {
    ...
    let respone = await match(selectedOnly)
      ...
      .with(true, async () => {
        return await api.Ser_MST_Service_ExportByListSerCode(selectedItems);
      })
      .otherwise(async () => {
        if (isHQ()) {
          return await api.Ser_MST_Service_ExportHQ({
            ...searchCondition,
          });
        } else {
          return await api.Ser_MST_Service_ExportDL({
            ...searchCondition,
          });
        }
      });
