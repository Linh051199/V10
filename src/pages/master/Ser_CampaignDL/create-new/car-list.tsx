...
export const CarList = forwardRef(({}: CarListProps, ref: any) => {
  ...
  const handleSelectedCars = (selectedCars: any[]) => {
    ...
    // console.log(ref)
    // ref as any
    ref?.current?.setPageData({
      DataList: [...carList],
    });
  };
  const deleteButtonAvailable = useVisibilityControl({ defaultVisible: false });
  const handleSelectionChanged = (e: any) => {
    if (ref) {
      const gridRef = ref as MutableRefObject<DataGrid>;
      if (gridRef && gridRef.current.instance.getSelectedRowKeys().length > 0) {
  ...
  return (
    <ScrollView>
      ...
      {/* <GridViewOne
        ...
        storeKey={"Ser_CampaignDL-create-car-list-columns"}
