...
import { SerCustomerList } from "./view-list";
...
const Header = ({ rightButtons }: HeaderProps) => {
  const { t } = useI18n("Ser_CampaignDLDetail");
...
export const Ser_CampaignDLView = () => {
  const { t } = useI18n("Ser_CampaignDLView");
  ...
  const { data, isLoading, remove } = useQuery({
    ...
    queryFn: async () => {
      if (params.code) {
        const response = await api.Ser_CampaignDL_DetailDL(params.code
        );
