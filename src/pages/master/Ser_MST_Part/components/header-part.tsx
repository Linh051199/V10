...
import { selectedItemsAtom } from "./ser-mst-part";
import { Search_Ser_MST_Part } from "@/packages/types/master/Ser_MST_Part";
import { usePermissions } from "@/packages/contexts/permission";

interface HeaderPartProps {
  onAddNew: () => void;
  searchCondition: Partial<Search_Ser_MST_Part>;
  handleRefetch: () => void;
}
export const HeaderPart = ({
  ...
  const onDownloadTemplate = async () => {
    const respone = await api.Ser_MST_Part_Export_Template();
  ...
  const handleUploadFiles = async (files: File[]) => {
    const respone = await api.Ser_MST_Part_Upload(files[0]);
    if (respone.isSuccess) {
      closeButton();
      handleRefetch();
  ...
  const handleExportExcel = async (selectedOnly: boolean) => {
    ...
    let respone = await match(selectedOnly)
      ...
      .with(true, async () => {
        return await api.Ser_MST_Part_ExportByListPartCode(selectedItems);
      })
      .otherwise(async () => {
        if (isHQ()) {
          return await api.Ser_MST_Part_ExportHQ({
            ...searchCondition,
          });
        } else {
          return await api.Ser_MST_Part_ExportDL({
            ...searchCondition,
          });
        }
      });
