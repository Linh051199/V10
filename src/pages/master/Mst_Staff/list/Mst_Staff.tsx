...
import { Search_Mst_Staff_Param } from "@/packages/types/master/Mst_Staff";
...
export const Mst_StaffPage = () => {
  const { t } = useI18n("Mst_Staff");

  const loadingControl = useVisibilityControl({ defaultVisible: false });

  const api = useClientgateApi();
  let gridRef: any = useRef<DataGrid | null>(null);
  const searchCondition = useRef<Partial<Search_Mst_Staff_Param>>({
  ...
  const fetchData = async () => {
    const resp: any = await api.Mst_Staff_Search(searchCondition.current);
            ...
            {!loadingControl.visible && (
              <>
                <GridViewOne
                  ...
                  keyExpr={"StaffCode"}
                  storeKey={"Mst_Staff-management-columns"}
                />
              </>
            )}
          </ContentSearchPanelLayout.Slot>
