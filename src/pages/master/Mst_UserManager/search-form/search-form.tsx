...
const SearchForm = ({ data, onSearch }: SearchFormProps) => {
  const { t } = useI18n("MST_UserManager_form");
  ...
  const searchFields: any[] = [
    ...
    {
      visible: true,
      dataField: "FlagActiveListForUser",
      label: {
        text: t("FlagActiveListForUser"),
      },
      render: (param: any) => {
        const { dataField, component: formComponent } = param;
        const formData = formComponent.option("formData");
    ...
    {
      visible: true,
      dataField: "FlagActiveListForDealer",
      label: {
        text: t("FlagActiveListForDealer"),
      },
      render: (param: any) => {
        const { dataField, component: formComponent } = param;
        const formData = formComponent.option("formData");
  ...
  const { saveState, loadState } = useSavedState<ColumnOptions[]>({
    storeKey: "MST_UserManager_search-form",
  });
