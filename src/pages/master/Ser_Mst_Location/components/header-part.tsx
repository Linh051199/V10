...
import { selectedItemsAtom } from "./ser-mst-location";
import { Search_Ser_Mst_Location } from "@/packages/types/master/Ser_Mst_Location";
import { usePermissions } from "@/packages/contexts/permission";

interface HeaderPartProps {
  onAddNew: () => void;
  searchCondition: Partial<Search_Ser_Mst_Location>;
  handleRefetch: () => void;
}
export const HeaderPart = ({
  ...
  const onDownloadTemplate = async () => {
    const respone = await api.Ser_Mst_Location_Export_Template();
  ...
  const handleUploadFiles = async (files: File[]) => {
    const respone = await api.Ser_Mst_Location_Upload(files[0]);
    if (respone.isSuccess) {
      closeButton();
      handleRefetch();
  ...
  const handleExportExcel = async (selectedOnly: boolean) => {
    ...
    let respone = await match(selectedOnly)
      ...
      .with(true, async () => {
        return await api.Ser_Mst_Location_ExportByListLocationCode(
          selectedItems
        );
      })
      .otherwise(async () => {
        if (isHQ()) {
          return await api.Ser_Mst_Location_ExportHQ({
            ...searchCondition,
          });
        } else {
          return await api.Ser_Mst_Location_ExportDL({
            ...searchCondition,
          });
        }
      });
