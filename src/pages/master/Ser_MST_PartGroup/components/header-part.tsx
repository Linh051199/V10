...
import { Search_Ser_MST_PartGroup } from "@/packages/types/master/Ser_MST_PartGroup";
import { usePermissions } from "@/packages/contexts/permission";
import { selectedItemsAtom } from "./ser-mst-partgroup";

interface HeaderPartProps {
  onAddNew: () => void;
  searchCondition: Partial<Search_Ser_MST_PartGroup>;
  handleRefetch: () => void;
}
export const HeaderPart = ({
  ...
  const onDownloadTemplate = async () => {
    const respone = await api.Ser_MST_PartGroup_Export_Template();
  ...
  const handleUploadFiles = async (files: File[]) => {
    const respone = await api.Ser_MST_PartGroup_Upload(files[0]);
    if (respone.isSuccess) {
      closeButton();
      handleRefetch();
  ...
  const handleExportExcel = async (selectedOnly: boolean) => {
    ...
    let respone = await match(selectedOnly)
      ...
      .with(true, async () => {
        return await api.Ser_MST_PartGroup_ExportByListPartGroupID(
          selectedItems
        );
      })
      .otherwise(async () => {
        if (isHQ()) {
          return await api.Ser_MST_PartGroup_ExportHQ({
            ...searchCondition,
          });
        } else {
          return await api.Ser_MST_PartGroup_ExportDL({
            ...searchCondition,
          });
        }
      });
    ...
    <PageHeaderNoSearchLayout>
      <PageHeaderNoSearchLayout.Slot name={"Before"}>
        <div className="font-bold dx-font-m">
          {t("Ser_MST_PartGroup Management")}
        </div>
