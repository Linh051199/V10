...
import { Search_SerInvReportTotalStockInDetailRpt_Params } from "@/packages/types/report/SerInvReportTotalStockInDetailRpt";
...
export const SerInvReportTotalStockInDetailRptPage = () => {
  const { t } = useI18n("SerInvReportTotalStockInDetailRpt");
  ...
  const fetchData = async () => {
    const resp = await match(isHQ())
      .with(true, async () => {
        const resp = await api.SerInvReportTotalStockInDetailRpt_SearchHQ({
      ...
      .with(false, async () => {
        const resp = await api.SerInvReportTotalStockInDetailRpt_SearchDL({
          DealerCode: DealerCode,
          FromDate: searchCondition.current.FromDateFromTo[0]
            ? format(
                searchCondition.current.FromDateFromTo[0] as Date,
    ...
    const length =
      resp?.Data?.lst_Ser_InvReportTotalStockInDetailRpt?.length ?? 0;

    if (resp?.isSuccess) {
      return {
        DataList: resp?.Data?.lst_Ser_InvReportTotalStockInDetailRpt,
  ...
  const { data: Dealer_GetAllActive, isLoading } = useQuery(
    ["Dealer_GetAllActive-SerInvReportTotalStockInDetailRptHTV"],
    async () => {
  ...
  const handleExport = async () => {
    ...
    if (crData && crData?.length > 0) {
      ...
      const resp = await match(isHQ())
        .with(true, async () => {
          const resp =
            await api.SerInvReportTotalStockInDetailRpt_ExportSearchHQ({
              DealerCode: searchCondition?.current?.DealerCode ?? "",
              FromDate: searchCondition.current.FromDateFromTo[0]
                ? format(
                    searchCondition.current.FromDateFromTo[0] as Date,
        ...
        .with(false, async () => {
          const resp =
            await api.SerInvReportTotalStockInDetailRpt_ExportSearchDL({
              DealerCode: DealerCode,
              FromDate: searchCondition.current.FromDateFromTo[0]
                ? format(
                    searchCondition.current.FromDateFromTo[0] as Date,
  ...
  const handlePrint = async () => {
    ...
    if (crData && crData?.length > 0) {
      ...
      const resp = await match(isHQ())
        .with(true, async () => {
          const resp = await api.SerInvReportTotalStockInDetailRpt_PrintHQ({
            DealerCode: searchCondition?.current?.DealerCode ?? "",
            FromDate: searchCondition.current.FromDateFromTo[0]
              ? format(
                  searchCondition.current.FromDateFromTo[0] as Date,
        ...
        .with(false, async () => {
          const resp = await api.SerInvReportTotalStockInDetailRpt_PrintDL({
            DealerCode: DealerCode,
            FromDate: searchCondition.current.FromDateFromTo[0]
              ? format(
                  searchCondition.current.FromDateFromTo[0] as Date,
        ...
        <ContentSearchPanelLayout>
            ...
            <div className={"w-[300px] h-full"}>
              {DealerCode === "HTC" ? (
                <SearchPanelV2
                  ...
                  storeKey={"SerInvReportTotalStockInDetailRptHTC-search"}
                />
              ) : (
                <SearchPanelV2
                  ...
                  onSearch={handleSearch}
                  storeKey={"SerInvReportTotalStockInDetailRpt-search"}
                />
              )}
            </div>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <GridViewOne
              ...
              isLoading={false}
              customToolbarItems={[]}
              storeKey={"SerInvReportTotalStockInDetailRpt-columns"}
              editMode={false}
