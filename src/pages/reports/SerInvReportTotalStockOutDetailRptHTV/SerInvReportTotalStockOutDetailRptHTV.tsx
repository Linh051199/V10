...
export const SerInvReportTotalStockOutDetailRptHTVPage = () => {
  const { t } = useI18n("SerInvReportTotalStockOutDetailRptHTV");
  ...
  const handleExportDetail = async () => {
    ...
    //     FromDate: searchCondition.current.FromDateTo[0]
    //       ? format(
    //           searchCondition.current.FromDateTo[0] as Date,
    //           "yyyy-MM-dd"
    //         )
    //       : "",
    //     ToDate: searchCondition.current.FromDateTo[1]
    //       ? format(
    //           searchCondition.current.FromDateTo[1] as Date,
    //           "yyyy-MM-dd"
    //         )
    //       : "",
  ...
  const handleExport = async () => {
    const resp = await api.SerInvReportTotalStockOutDetailRpt_ExportSearchHQ({
      FromDate: searchCondition.current.FromDateFromTo[0]
        ? format(
            searchCondition.current.FromDateFromTo[0] as Date,
            "yyyy-MM-dd"
  ...
  const handlePrint = async () => {
    const crData = gridRef?.current?.getData();
    if (crData && crData?.length > 0) {
      const response = await api.SerInvReportTotalStockOutDetailRpt_PrintHQ({
        FromDate: searchCondition.current.FromDateFromTo[0]
          ? format(
              searchCondition.current.FromDateFromTo[0] as Date,
              "yyyy-MM-dd"
        ...
        <ContentSearchPanelLayout>
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <div className={"w-[300px] h-full"}>
              <SearchPanelV2
                ...
                storeKey={"SerInvReportTotalStockOutDetailRptHTV-search"}
              />
            </div>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <GridViewOne
              ...
              isLoading={false}
              customToolbarItems={[]}
              storeKey={"SerInvReportTotalStockOutDetailRptHTV-columns"}
              editMode={false}
