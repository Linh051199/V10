...
export const Rpt_PivotChamTonToiThieuPage = () => {
  const { t } = useI18n("Rpt_PivotChamTonToiThieu");
  ...
  const { data, isLoading } = useQuery({
    queryKey: [
      "report",
      "Rpt_PivotChamTonToiThieu",
      loadingKey,
      JSON.stringify(searchCondition),
    ],
    queryFn: async () => {
      if (loadingKey !== "0") {
        // const resp = await api.Rpt_PivotChamTonToiThieu_SearchHQ({
        ...
        // } as Rpt_PivotChamTonToiThieuParam);
        // return resp;
      } else {
        return null;
      }
  ...
  const handleExportDetail = async () => {
    if (loadingKey !== "0") {
      setGettingData(true);
      // const result = await api.Rpt_PivotChamTonToiThieu_ExportDetailSearchHQ({
  ...
  const onExportExcelByPivot = useCallback(
    (e: ExportingEvent) => {
      if (loadingKey !== "0") {
        commonExportExcel(
          e,
          "Rpt_PivotChamTonToiThieu_ExportExcelByPivot.xlsx",
          t
        );
        e.cancel = true;
      } else {
  ...
  const dataSource = new PivotGridDataSource({
    fields: fields,
    // store: data?.Data?.Lst_Rpt_PivotChamTonToiThieu,
  });
  const handleChangeNoData = (args: ContentReadyEvent) => {
    // const checkData = data?.Data?.Lst_Rpt_PivotChamTonToiThieu;
          ...
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <div className={"w-[300px] h-full"}>
              <SearchPanelV2
                ...
                storeKey={"Rpt_PivotChamTonToiThieu_search"}
              />
            </div>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            ...
            <div className={"w-full mt-4"}>
              ...
              >
                ...
                <StateStoring
                  enabled={true}
                  storageKey={"report-Rpt_PivotChamTonToiThieu_columns"}
                />
