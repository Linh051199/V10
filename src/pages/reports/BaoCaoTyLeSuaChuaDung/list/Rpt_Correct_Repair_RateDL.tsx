...
export const Rpt_Correct_Repair_RateDLPage = () => {
  ...
  const handleExport = async () => {
    const response = await match(isHQ())
      ...
      .otherwise(async () => {
        const response = await api.Rpt_Correct_Repair_Rate_ExportSearchDL({
          DealerCode: searchCondition.current.DealerCode ?? "",
          FromDate: searchCondition.current.DateFromTo[0]
            ? format(searchCondition.current.DateFromTo[0], "yyyy-MM-dd")
            : "",
    ...
    const resp = await match(isHQ())
      .with(true, async () => {
        const response = await api.Rpt_Correct_Repair_Rate_SearchHQ({
          ...
          FromDate: searchCondition.current.DateFromTo?.[0]
            ? format(
                searchCondition.current.DateFromTo[0] as Date,
                "yyyy-MM-dd"
              )
            : "",
          ToDate: searchCondition.current.DateFromTo?.[1]
            ? format(
                searchCondition.current.DateFromTo[1] as Date,
                "yyyy-MM-dd"
              )
            : "",
      ...
      .otherwise(async () => {
        const response = await api.Rpt_Correct_Repair_Rate_SearchDL({
          FromDate: searchCondition.current.DateFromTo?.[0]
            ? format(
                searchCondition.current.DateFromTo[0] as Date,
                "yyyy-MM-dd"
