...
import { Search_SerInvReportInsuranceDebitRpt_Params } from "@/packages/types/report/SerInvReportInsuranceDebitRpt";
...
export const SerInvReportInsuranceDebitRptPage = () => {
  ...
  const fetchData = async () => {
    const resp = await api.SerInvReportInsuranceDebitRpt_SearchDL({
    ...
    const length = resp?.Data?.Lst_Ser_InvReportInsuranceDebitRpt?.length ?? 0;

    if (resp?.isSuccess) {
      return {
        DataList: resp?.Data?.Lst_Ser_InvReportInsuranceDebitRpt,
  ...
  const handleExport = async () => {
    ...
    if (crData && crData?.length > 0) {
      setLoad(true);
      const response = await api.SerInvReportInsuranceDebitRpt_ExportSearchDL({
        DealerCode: DealerCode,
        FromDate: searchCondition.current.FromDateFromTo[0]
          ? format(
              searchCondition.current.FromDateFromTo[0] as Date,
  ...
  const handlePrint = async () => {
    ...
    if (crData && crData?.length > 0) {
      setLoad(true);
      const response = await api.SerInvReportInsuranceDebitRpt_PrintDL({
        DealerCode: DealerCode,
        FromDate: searchCondition.current.FromDateFromTo[0]
          ? format(
              searchCondition.current.FromDateFromTo[0] as Date,
        ...
        <ContentSearchPanelLayout>
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <div className={"w-[300px] h-full"}>
              <SearchPanelV2
                ...
                storeKey={"SerInvReportInsuranceDebitRpt-search"}
              />
            </div>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <GridViewOne
              ...
              isLoading={false}
              customToolbarItems={[]}
              storeKey={"SerInvReportInsuranceDebitRpt-columns"}
              editMode={false}
