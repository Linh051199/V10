...
export const Ser_Report_Customer_Not_BackHQPage = () => {
  ...
  const handleExport = async () => {
    const response = await api.Ser_Report_Customer_Not_Back_ExportSearchHQ({
  ...
  const fetchData = async () => {
    const resp = await api.Ser_Report_Customer_Not_Back_SearchHQ({
      DealerCode: searchCondition.current.DealerCode ?? "",
      DateCount: searchCondition.current.DateCount ?? 0,
      FlagDataWH: searchCondition.current.FlagDataWH ? 1 : 0,
    } as Ser_Report_Customer_Not_BackParam);
