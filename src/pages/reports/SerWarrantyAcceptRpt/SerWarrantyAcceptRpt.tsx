...
import { Search_SerWarrantyAcceptRpt_Params } from "@/packages/types/report/SerWarrantyAcceptRpt";
...
export const SerWarrantyAcceptRptPage = () => {
  ...
  const searchCondition = useRef<Partial<Search_SerWarrantyAcceptRpt_Params>>({
  ...
  const fetchData = async () => {
    const resp = await match(isHQ())
      .with(true, async () => {
        const resp = await api.SerWarrantyAcceptRpt_SearchHQ({
      ...
      .with(false, async () => {
        const resp = await api.SerWarrantyAcceptRpt_SearchDL({
          DealerCode: DealerCode,
          FromDate: searchCondition.current.FromDateFromTo[0]
            ? format(
                searchCondition.current.FromDateFromTo[0] as Date,
    ...
    const length = resp?.Data?.length ?? 0;
  ...
  //   ["Dealer_GetAllActive-SerWarrantyAcceptRpt"],
  //   async () => {
  ...
  const handleExportDetail = async () => {
    ...
    if (crData && crData?.length > 0) {
      setLoad(true);
      const resp = await api.SerWarrantyAcceptRpt_ExportDetailSearchHQ({
        DealerCode: searchCondition?.current?.DealerCode ?? "",
        FromDate: searchCondition.current.FromDateFromTo[0]
          ? format(
              searchCondition.current.FromDateFromTo[0] as Date,
  ...
  const handleExport = async () => {
    ...
    if (crData && crData?.length > 0) {
      setLoad(true);
      const resp = await api.SerWarrantyAcceptRpt_ExportSearchHQ({
        DealerCode: searchCondition?.current?.DealerCode ?? "",
        FromDate: searchCondition.current.FromDateFromTo[0]
          ? format(
              searchCondition.current.FromDateFromTo[0] as Date,
  ...
  const handlePrint = async () => {
    ...
    if (crData && crData?.length > 0) {
      setLoad(true);
      const resp = await api.SerWarrantyAcceptRpt_PrintHQ({
        DealerCode: searchCondition?.current?.DealerCode ?? "",
        FromDate: searchCondition.current.FromDateFromTo[0]
          ? format(
              searchCondition.current.FromDateFromTo[0] as Date,
        ...
        <ContentSearchPanelLayout>
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <div className={"w-[300px] h-full"}>
              <SearchPanelV2
                ...
                storeKey={"SerWarrantyAcceptRpt-search"}
              />
            </div>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <GridViewOne
              ...
              isLoading={false}
              customToolbarItems={[]}
              storeKey={"SerWarrantyAcceptRpt-columns"}
              editMode={false}
