...
import { Search_SerInvReportTotalStockOutRpt_Params } from "@/packages/types/report/SerInvReportTotalStockOutRpt";
...
export const SerInvReportTotalStockOutRptPage = () => {
  ...
  const fetchData = async () => {
    const resp = await api.SerInvReportTotalStockOutRpt_SearchDL({
    ...
    const length = resp?.Data?.Lst_Ser_InvReportTotalStockOutRpt?.length ?? 0;

    if (resp?.isSuccess) {
      return {
        DataList: resp?.Data?.Lst_Ser_InvReportTotalStockOutRpt,
  ...
  const handleExport = async () => {
    ...
    if (crData && crData?.length > 0) {
      setLoad(true);
      const resp = await api.SerInvReportTotalStockOutRpt_ExportSearchDL({
        DealerCode: DealerCode,
        FromDate: searchCondition.current.FromDateFromTo[0]
          ? format(
              searchCondition.current.FromDateFromTo[0] as Date,
  ...
  const handlePrint = async () => {
    ...
    if (crData && crData?.length > 0) {
      setLoad(true);
      const response = await api.SerInvReportTotalStockOutRpt_PrintDL({
        DealerCode: DealerCode,
        FromDate: searchCondition.current.FromDateFromTo[0]
          ? format(
              searchCondition.current.FromDateFromTo[0] as Date,
        ...
        <ContentSearchPanelLayout>
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <div className={"w-[300px] h-full"}>
              <SearchPanelV2
                ...
                storeKey={"SerInvReportTotalStockOutRpt-search"}
              />
            </div>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <GridViewOne
              ...
              isLoading={false}
              customToolbarItems={[]}
              storeKey={"SerInvReportTotalStockOutRpt-columns"}
