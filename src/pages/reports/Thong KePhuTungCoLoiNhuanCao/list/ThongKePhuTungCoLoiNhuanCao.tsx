...
import { useRpt_ThongKePhuTungCoLoiNhuanCaoColumns } from "../components/useReportColumns";
...
export const Rpt_ThongKePhuTungCoLoiNhuanCaoPage = () => {
  const { t } = useI18n("Rpt_ThongKePhuTungCoLoiNhuanCaoPage");
  ...
  const columns = useRpt_ThongKePhuTungCoLoiNhuanCaoColumns({
    data: [],
  });

  const fetchData = async () => {
    const resp = await api.Ser_InvReportPartTopProfit_SearchDL({
      ...searchCondition.current,
      FromDate: searchCondition?.current?.ToFrom[0]
        ? typeof searchCondition?.current?.ToFrom[0] === "object"
          ? format(searchCondition.current?.ToFrom[0], "yyyy-MM-dd")
    ...
    if (resp?.isSuccess) {
      return {
        DataList: resp?.Data?.lst_Ser_InvReportPartTopProfit || [],
      };
    } else {
      showError({
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <ScrollView height={windowSize.height - 120}>
              <GridViewOne
                ...
                isLoading={false}
                customToolbarItems={[]}
                storeKey={"Rpt_ThongKePhuTungCoLoiNhuanCao"}
                editMode={false}
