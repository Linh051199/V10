...
export const SearchForm = forwardRef(
  ({ data, onSearch }: SearchFormProps, ref: any) => {
    const { t } = useI18n("Ser_ReportHistoryCost-search-form");
    const setSearchPanelVisible = useSetAtom(searchPanelVisibleAtom);
    const [dataValue, setDataValue] = useState({});
    const { saveState, loadState } = useSavedState<ColumnOptions[]>({
      storeKey: "Ser_ReportHistoryCost_Form",
    });
        ...
        render: (param: any) => {
          ...
          return (
            <DateRangeBox
              ...
              defaultStartDate={
                formData.DateFromTo[0]
                  ? typeof formData.DateFromTo[0] === "object"
                    ? format(formData.DateFromTo[0], "yyyy-MM-dd")
                    : formData.DateFromTo[0]
                  : ""
              }
              defaultEndDate={
                formData.DateFromTo[1]
                  ? typeof formData.DateFromTo[1] === "object"
                    ? format(formData.DateFromTo[1], "yyyy-MM-dd")
                    : formData.DateFromTo[1]
                  : ""
              }
