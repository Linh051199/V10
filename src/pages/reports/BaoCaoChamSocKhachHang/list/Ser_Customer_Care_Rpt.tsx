...
import { useSer_Customer_Care_RptColumns } from "../components/useReportColumns";
...
import { Ser_Customer_Care_RptParam } from "@/packages/api/clientgate/report/Ser_Customer_Care_RptApi";

export const Ser_Customer_Care_RptPage = () => {
  ...
  const searchCondition = useRef<Ser_Customer_Care_RptParam>({
    FlagDataWH: 0,
    DateFromTo: [validateMajorTimeStartDayOfMonth, new Date()],
  } as Ser_Customer_Care_RptParam);
  ...
  const columns = useSer_Customer_Care_RptColumns({
    data: [],
  });
  const { saveState, loadState } = useSavedState<ColumnOptions[]>({
    storeKey: "Ser_Customer_Care_Rpt-columns",
  });
  ...
  const fetchData = async () => {
    const resp = await api.Ser_Customer_Care_Rpt_SearchDL({
    ...
    const length = resp?.Data?.Lst_Ser_CustomerCareRpt?.length ?? 0;

    if (resp?.isSuccess) {
      return {
        DataList: resp?.Data?.Lst_Ser_CustomerCareRpt,
  ...
  return (
    <AdminContentLayout className={"province-management"}>
      <AdminContentLayout.Slot name={"Header"}>
        <PageHeader toggleSearchPanel={handleToggleSearchPanel}></PageHeader>
          ...
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <div className={"w-[300px]"}>
              <SearchPanelV2
                ...
                storeKey={"Ser_Customer_Care_Rpt-search"}
              />
            </div>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            ...
            <ScrollView height={windowSize.height - 120}>
              <GridViewOne
                ...
                isLoading={false}
                customToolbarItems={[]}
                storeKey={"Ser_Customer_Care_Rpt"}
                editMode={false}
