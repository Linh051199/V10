...
export const SerInventoryReportInOutBalanceHTVPage = () => {
  const { t } = useI18n("SerInventoryReportInOutBalanceHTV");
  ...
  const handlePrint = async () => {
    ...
    if (crData && crData?.length > 0) {
      const response = await api.SerInventoryReportInOutBalance_PrintHQ({
        FromDate: searchCondition.current.FromDateFromTo[0]
          ? format(
              searchCondition.current.FromDateFromTo[0] as Date,
              "yyyy-MM-dd"
        ...
        <ContentSearchPanelLayout>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <GridViewOne
              ...
              isHidenHeaderFilter
              customToolbarItems={[]}
              storeKey={"SerInventoryReportInOutBalanceHTV-columns"}
              editMode={false}
