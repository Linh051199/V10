...
import { useRpt_ThongKePhuTungLuanChuyenNhanhColumns } from "../components/useReportColumns";
...
export const Rpt_ThongKePhuTungLuanChuyenNhanhPage = () => {
  const { t } = useI18n("Rpt_ThongKePhuTungLuanChuyenNhanhPage");
  ...
  const columns = useRpt_ThongKePhuTungLuanChuyenNhanhColumns({
    data: [],
  });

  const fetchData = async () => {
    const resp = await api.Ser_InvReportPartTopRotate_SearchDL({
      ...searchCondition.current,
      FromDate: searchCondition?.current?.ToFrom[0]
        ? typeof searchCondition?.current?.ToFrom[0] === "object"
          ? format(searchCondition.current?.ToFrom[0], "yyyy-MM-dd")
    ...
    if (resp?.isSuccess) {
      return {
        DataList: resp?.Data?.lst_Ser_InvReportPartTopRotate || [],
      };
    } else {
      showError({
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <ScrollView height={windowSize.height - 120}>
              <GridViewOne
                ...
                isLoading={false}
                customToolbarItems={[]}
                storeKey={"Rpt_ThongKePhuTungLuanChuyenNhanhPage"}
                editMode={false}
