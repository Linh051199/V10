...
import { useRpt_Correct_Repair_RateColumns } from "../BaoCaoTyLeSuaChuaDung/components/useReportColumns";

export const RptCorrectRepairRateHTVPage = () => {
  const { t } = useI18n("RptCorrectRepairRateHTV");
  ...
  const { saveState, loadState } = useSavedState<ColumnOptions[]>({
    storeKey: "RptCorrectRepairRateHTV-columns",
  });
  ...
  const fetchData = async () => {
    const resp = await api.Rpt_Correct_Repair_Rate_SearchHQ({
    ...
    const length = resp?.Data?.Lst_Rpt_Correct_Repair_RateDtl?.length ?? 0;

    if (resp?.isSuccess) {
      headerFormViewRef?.current?.setDataForm(resp?.Data);
      return {
        DataList: resp?.Data?.Lst_Rpt_Correct_Repair_RateDtl,
          ...
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <div className={"w-[300px] h-full"}>
              <SearchPanelV2
                ...
                storeKey={"RptCorrectRepairRateHTV-search"}
              />
            </div>
