...
import { useSer_Count_CustomerColumns } from "../components/useReportColumns";
...
import { Ser_Count_CustomerParam } from "@/packages/api/clientgate/report/Ser_Count_CustomerApi";
...
export const Ser_Count_CustomerPage = () => {
  ...
  const handleExport = async () => {
    setGettingData(true);
    const response = await api.Ser_Count_Customer_ExportSearchDL({
  ...
  const handlePrint = async () => {
    const response = await api.Ser_Count_Customer_PrintDL({
      DealerCode: searchCondition.current.DealerCode ?? "",
      TradeMarkCode: searchCondition.current.TradeMarkCode ? 1 : 0,
      FromDate: searchCondition.current.DateFromTo[0]
        ? format(searchCondition.current.DateFromTo[0], "yyyy-MM-dd")
  ...
  const columns = useSer_Count_CustomerColumns({
    data: [],
  });
  const { saveState, loadState } = useSavedState<ColumnOptions[]>({
    storeKey: "Ser_Count_Customer-columns",
  });
  ...
  const fetchData = async () => {
    const resp = await api.Ser_Count_Customer_SearchDL({
      DealerCode: searchCondition.current.DealerCode ?? "",
      TradeMarkCode: searchCondition.current.TradeMarkCode ? 1 : 0,
      FromDate: searchCondition.current.DateFromTo[0]
        ? format(searchCondition.current.DateFromTo[0], "yyyy-MM-dd")
    ...
    const length = resp?.Data?.lst_Ser_Count_Customer?.length ?? 0;

    if (resp?.isSuccess) {
      return {
        DataList: resp?.Data?.lst_Ser_Count_Customer,
          ...
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <div className={"w-[300px]"}>
              <SearchPanelV2
                ...
                storeKey={"Ser_Count_Customer-search"}
              />
            </div>
