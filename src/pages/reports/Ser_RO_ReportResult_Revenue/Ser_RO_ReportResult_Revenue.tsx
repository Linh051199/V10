...
import { Search_Ser_RO_ReportResult_Revenue_Params } from "@/packages/types/report/Ser_RO_ReportResult_Revenue";
import { format } from "date-fns";
import { Summary, TotalItem } from "devextreme-react/data-grid";
import { loadPanelAtom } from "@/packages/store/loadPanel-store";
import usePrint from "@/components/print/usePrint";

export const Ser_RO_ReportResult_RevenuePage = () => {
  ...
  >({
    ...
    FromDatePaidFromTo: [validateTimeStartDayOfMonth, new Date()],
    FlagDataWH: false,
  });
  //====================================CallAPI========================================
  const fetchData = async () => {
    const resp = await api.Ser_RO_ReportResult_RevenueRpt_SearchDL({
      ...
      ToDate: searchCondition.current.FromDateFromTo[1]
        ...
        : "",
      FromDatePaid: searchCondition.current.FromDatePaidFromTo[0]
        ? format(
            searchCondition.current.FromDatePaidFromTo[0] as Date,
            "yyyy-MM-dd"
          )
        : "",
      ToDatePaid: searchCondition.current.FromDatePaidFromTo[1]
        ? format(
            searchCondition.current.FromDatePaidFromTo[1] as Date,
            "yyyy-MM-dd"
          )
        : "",
    ...
    const length = resp?.Data?.lst_Ser_RO_ReportResult_Revenue?.length ?? 0;

    if (resp?.isSuccess) {
      return {
        DataList: resp?.Data?.lst_Ser_RO_ReportResult_Revenue,
  ...
  const handleSearch = async (data: any) => {
    searchCondition.current = {
      ...
      FromDatePaid: data.FromDatePaidFromTo[0]
        ? data.FromDatePaidFromTo[0]
        : "",
      ToDatePaid: data.FromDatePaidFromTo[1] ? data.FromDatePaidFromTo[1] : "",
    };
    if (data.FlagDataWH) {
      setOpenPopupWH(true);
    } else {
  ...
  const handlePrint = async () => {
    ...
    if (crData && crData?.length > 0) {
      setLoad(true);
      const response = await api.Ser_RO_ReportResult_RevenueRpt_PrintDL({
        FromDate: searchCondition.current.FromDateFromTo[0]
          ? format(
              searchCondition.current.FromDateFromTo[0] as Date,
              "yyyy-MM-dd"
  ...
  const handleExport = async () => {
    ...
    if (crData && crData?.length > 0) {
      setLoad(true);
      const response = await api.Ser_RO_ReportResult_RevenueRpt_ExportSearchDL({
        FromDate: searchCondition.current.FromDateFromTo[0]
          ? format(
              searchCondition.current.FromDateFromTo[0] as Date,
              "yyyy-MM-dd"
  ...
  const searchFields: IItemProps[] = [
    ...
    {
      dataField: "FromDatePaidFromTo",
      caption: t("FromDatePaidFromTo"),
      visible: true,
      render: (param: any) => {
        const { dataField, component: formComponent } = param;
        return (
          <div className={"flex flex-row"}>
            <DateRangeBox
              ...
              displayFormat=" yyyy-MM-dd"
              showClearButton={true}
              defaultStartDate={searchCondition?.current.FromDatePaidFromTo[0]}
              defaultEndDate={searchCondition?.current.FromDatePaidFromTo[1]}
              useMaskBehavior={true}
              openOnFieldClick={true}
              labelMode="hidden"
              onValueChanged={(e: any) => {
        ...
        <ContentSearchPanelLayout>
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <div className={"w-[300px] h-full"}>
              <SearchPanelV2
                ...
                storeKey={"Ser_RO_ReportResult_Revenue-search"}
              />
            </div>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <GridViewOne
              ...
              isLoading={false}
              customToolbarItems={[]}
              storeKey={"Ser_RO_ReportResult_Revenue-columns"}
