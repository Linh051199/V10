...
import { useSer_InvReportPartTopVariationPriceColumns } from "../components/useReportColumns";
import { Ser_InvReportPartTopVariationPriceParam } from "@/packages/api/clientgate/report/Ser_InvReportPartTopVariationPriceApi";

export const Ser_InvReportPartTopVariationPricePage = () => {
  ...
  const searchCondition = useRef<Ser_InvReportPartTopVariationPriceParam>({
    Top: "",
    DealerCode: "",
    DateFromTo: [validateMajorTimeStartDayOfMonth, new Date()],
  } as Ser_InvReportPartTopVariationPriceParam);
  ...
  const handleExport = async () => {
    const response =
      await api.Ser_InvReportPartTopVariationPrice_ExportSearchDL({
        DealerCode: searchCondition.current.DealerCode ?? "",
        Top: searchCondition.current.Top ?? "",
  ...
  const columns = useSer_InvReportPartTopVariationPriceColumns({
    data: [],
  });
  const { saveState, loadState } = useSavedState<ColumnOptions[]>({
    storeKey: "Ser_InvReportPartTopVariationPrice-columns",
  });
  ...
  const fetchData = async () => {
    const resp = await api.Ser_InvReportPartTopVariationPrice_SearchDL({
      DealerCode: searchCondition.current.DealerCode ?? "",
      Top: searchCondition.current.Top ?? "",
      FromDate: searchCondition.current.DateFromTo[0]
        ? format(searchCondition.current.DateFromTo[0], "yyyy-MM-dd")
    ...
    const length =
      resp?.Data?.lst_Ser_InvReportPartTopVariationPrice?.length ?? 0;

    if (resp?.isSuccess) {
      return {
        DataList: resp?.Data?.lst_Ser_InvReportPartTopVariationPrice,
          ...
          <ContentSearchPanelLayout.Slot name={"SearchPanel"}>
            <div className={"w-[300px]"}>
              <SearchPanelV2
                ...
                storeKey={"Ser_InvReportPartTopVariationPrice-search"}
              />
            </div>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            ...
            <ScrollView height={windowSize.height - 120}>
              <GridViewOne
                ...
                isLoading={false}
                customToolbarItems={[]}
                storeKey={"Ser_InvReportPartTopVariationPrice"}
                editMode={false}
