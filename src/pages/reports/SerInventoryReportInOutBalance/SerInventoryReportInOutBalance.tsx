...
import { Search_SerInventoryReportInOutBalance_Params } from "@/packages/types/report/SerInventoryReportInOutBalance";
...
export const SerInventoryReportInOutBalancePage = () => {
  ...
  const fetchData = async () => {
    const resp = await match(isHQ())
      .with(true, async () => {
        const resp = await api.SerInventoryReportInOutBalance_SearchHQ({
      ...
      .with(false, async () => {
        const resp = await api.SerInventoryReportInOutBalance_SearchDL({
          DealerCode: DealerCode,
          FromDate: searchCondition.current.FromDateFromTo[0]
            ? format(
                searchCondition.current.FromDateFromTo[0] as Date,
    ...
    const length =
      resp?.Data?.lst_Ser_InventoryReport_InOutBalance?.length ?? 0;

    if (resp?.isSuccess) {
      return {
        DataList: resp?.Data?.lst_Ser_InventoryReport_InOutBalance,
  ...
  const { data: Dealer_GetAllActive, isLoading } = useQuery(
    ["Dealer_GetAllActive-SerInventoryReportInOutBalanceHTV"],
    async () => {
  ...
  const handleExport = async () => {
    ...
    if (crData && crData?.length > 0) {
      ...
      const resp = await match(isHQ())
        .with(true, async () => {
          const resp = await api.SerInventoryReportInOutBalance_ExportSearchHQ({
            FromDate: searchCondition.current.FromDateFromTo[0]
              ? format(
                  searchCondition.current.FromDateFromTo[0] as Date,
                  "yyyy-MM-dd"
        ...
        .with(false, async () => {
          const resp = await api.SerInventoryReportInOutBalance_ExportSearchDL({
            DealerCode: DealerCode,
            FromDate: searchCondition.current.FromDateFromTo[0]
              ? format(
                  searchCondition.current.FromDateFromTo[0] as Date,
  ...
  const handlePrint = async () => {
    ...
    if (crData && crData?.length > 0) {
      ...
      const resp = await match(isHQ())
        .with(true, async () => {
          const resp = await api.SerInventoryReportInOutBalance_PrintHQ({
            DealerCode: searchCondition?.current?.DealerCode ?? "",
            FromDate: searchCondition.current.FromDateFromTo[0]
              ? format(
                  searchCondition.current.FromDateFromTo[0] as Date,
        ...
        .with(false, async () => {
          const resp = await api.SerInventoryReportInOutBalance_PrintDL({
            DealerCode: DealerCode,
            FromDate: searchCondition.current.FromDateFromTo[0]
              ? format(
                  searchCondition.current.FromDateFromTo[0] as Date,
        ...
        <ContentSearchPanelLayout>
            ...
            <div className={"w-[300px] h-full"}>
              {DealerCode === "HTC" ? (
                <SearchPanelV2
                  ...
                  storeKey={"SerInventoryReportInOutBalanceHTV-search"}
                />
              ) : (
                <SearchPanelV2
                  ...
                  onSearch={handleSearch}
                  storeKey={"SerInventoryReportInOutBalance-search"}
                />
              )}
            </div>
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <GridViewOne
              ...
              isHidenHeaderFilter
              customToolbarItems={[]}
              storeKey={"SerInventoryReportInOutBalance-columns"}
              editMode={false}
