...
const Header = ({ rightButtons }: HeaderProps) => {
  const { t } = useI18n("QuanLyKhieuNaiDonHangDetail");
...
const QuanLyKhieuNaiDonHangDetail = () => {
  const paramUrl = useParams();
  const { t } = useI18n("QuanLyKhieuNaiDonHang_Detail");
  ...
  const handleSend = async () => {
    ...
    if (isValid) {
      ...
      if (dataList.length) {
        ...
        // } else {
        //   let response;
        //   if (paramUrl?.action === "Detail") {
        //     response = await api.HQ_DMS40_Ord_SalesOrderRoot_DetailDL(param);
        //   } else {
  ...
  //       //       DetailDTime: new Date(Lst_DMS40_Ord_SalesOrderRoot.DetailDTime),
  //       //     };
...
export default QuanLyKhieuNaiDonHangDetail;
