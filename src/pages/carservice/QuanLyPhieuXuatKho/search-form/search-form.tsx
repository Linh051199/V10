...
const SearchForm = ({ data, onSearch }: SearchFormProps) => {
  ...
  const searchFields: any[] = [
    ...
    {
      dataField: "NgayLapFromTo",
      caption: t("NgayLapFromTo"),
      visible: true,
      render: (param: any) => {
        const { dataField, component: formComponent } = param;
        const formData = formComponent.option("formData");
      ...
      render: (param: any) => {
        ...
        return (
          <div className={"flex flex-col list-checkbox"}>
            ...
            />
            <CheckboxField
              label={t("DangTienHanh")}
              dataField={"DangTienHanh"}
              formInstance={formComponent}
              defaultValue={formData?.["DangTienHanh"] == 1 ? true : false}
              onValueChanged={(e: any) => {
                formComponent.updateData("DangTienHanh", e.value);
              }}
            />
            <CheckboxField
              label={t("KetThuc")}
              dataField={"KetThuc"}
              formInstance={formComponent}
              defaultValue={formData?.["KetThuc"] == 1 ? true : false}
              onValueChanged={(e: any) => {
                formComponent.updateData("KetThuc", e.value);
              }}
            />
            <CheckboxField
              label={t("DaDieuChinh")}
              dataField={"DaDieuChinh"}
              formInstance={formComponent}
              defaultValue={formData?.["DaDieuChinh"] == 1 ? true : false}
              onValueChanged={(e: any) => {
                formComponent.updateData("DaDieuChinh", e.value);
              }}
            />
            <CheckboxField
              label={t("DaHuy")}
              dataField={"DaHuy"}
              formInstance={formComponent}
              defaultValue={formData?.["DaHuy"] == 1 ? true : false}
              onValueChanged={(e: any) => {
                formComponent.updateData("DaHuy", e.value);
              }}
            />
          </div>
        );
