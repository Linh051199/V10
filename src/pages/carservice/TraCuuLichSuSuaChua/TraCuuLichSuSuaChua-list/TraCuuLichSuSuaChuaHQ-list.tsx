...
import LookupRepairHistory from "../lookup-repair-history/lookup-repair-history";

interface TraCuuLichSuSuaChuaHQ_Search {}
...
const TraCuuLichSuSuaChuaHQPage = () => {
  const { t } = useI18n("TraCuuLichSuSuaChuaHQ");
  ...
  const searchCondition = useRef<Partial<TraCuuLichSuSuaChuaHQ_Search>>({
    BienSo: "",
    VIN: "",
    Ft_PageIndex: 0,
    Ft_PageSize: 1000,
  ...
  const gridLookupRepairHistory = useRef<any>(null);

  const navigate = useNetworkNavigate();
  const onViewDetail = (code: string) => {
    navigate(
      `/service/TraCuuLichSuSuaChuaHQ/manageTraCuuLichSuSuaChuaHQ/${code}?${searchCondition.current.FlagDataWH}`
    );
  };
  const { data, onSave: onSaveState } = useStateRestore<
    Partial<TraCuuLichSuSuaChuaHQ_Search>
  ...
  const handleSearch = (condition: Partial<TraCuuLichSuSuaChuaHQ_Search>) => {
  ...
  const handleExportExcel = async () => {
    ...
    // const response = await api.TraCuuLichSuSuaChuaHQ_PrintCBReqNo(
    //   searchCondition.current,
    //   key
    // );
  ...
  const handleXemKhieuNai = () => {
    gridLookupRepairHistory.current.show();
  };
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <ScrollView height={"100%"}>
              ...
              <GridViewOne
                ...
                keyExpr={"Dealer"}
                storeKey={"TraCuuLichSuSuaChuaHQ-list"}
              />
              <LookupRepairHistory
                ref={gridLookupRepairHistory}
                visible={false}
              />
            </ScrollView>
          </ContentSearchPanelLayout.Slot>
...
export default TraCuuLichSuSuaChuaHQPage;
