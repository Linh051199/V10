...
const SearchForm = ({ data, onSearch }: SearchFormProps) => {
  ...
  const { data: listSupplierName } = useQuery(
    ["listSupplierName", "Ser_Inv_StockIn"],
    async () => {
  ...
  const searchFields: any[] = [
    ...
    {
      dataField: "StockDateFromTo",
      label: {
        text: t("StockDateFromTo"),
      },
      // validationRules: [RequiredField(t("StockDateFromToIsRequired"))],
      render: (param: any) => {
        const { dataField, component: formComponent } = param;
        const formData = formComponent.option("formData");
        return (
      ...
      render: (param: any) => {
        ...
        return (
          <div className={"flex flex-row dms-form-field"}>
            <SelectField
              // readOnly={readOnly}
              // width={"100%"} // Thêm vào là bị kéo co lại
              dataField={dataField}
              ...
              // validationRules={[RequiredField(validateMsg("LoaiNhap"))]}
            />
          </div>
        );
      },
  ...
  const { saveState, loadState } = useSavedState<ColumnOptions[]>({
    storeKey: "ser-inv-stockin-admin-search-field",
  });
