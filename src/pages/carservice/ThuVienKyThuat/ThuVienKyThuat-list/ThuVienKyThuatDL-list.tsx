...
interface ThuVienKyThuatDL_Search {}
...
const ThuVienKyThuatDLPage = () => {
  const { t } = useI18n("ThuVienKyThuatDL");
  ...
  const searchCondition = useRef<Partial<ThuVienKyThuatDL_Search>>({
    Dealer: "",
    Model: "",
    DongCo: "",
    NoiDung: "",
  ...
  const onViewDetail = (code: string) => {
    navigate(
      `/service/ThuVienKyThuatDL/manageThuVienKyThuatDL/${code}?${searchCondition.current.FlagDataWH}`
    );
  };
  const { data, onSave: onSaveState } = useStateRestore<
    Partial<ThuVienKyThuatDL_Search>
  ...
  const handleSearch = (condition: Partial<ThuVienKyThuatDL_Search>) => {
  ...
  const fetchData = async () => {
    // const resp =   await api.ThuVienKyThuatDL_SearchHQ({
    //       ...searchCondition.current,
  ...
  const handleSave = async () => {
    ...
    // const response = await api.ThuVienKyThuatDL_PrintCBReqNo(
    //   searchCondition.current,
    //   key
    // );
          ...
          <ContentSearchPanelLayout.Slot name={"ContentPanel"}>
            <ScrollView height={"100%"}>
              ...
              <GridViewOne
                ...
                keyExpr={"MaKyThuat"}
                storeKey={"ThuVienKyThuatDL-list"}
              />
            </ScrollView>
          </ContentSearchPanelLayout.Slot>
        </ContentSearchPanelLayout>
...
export default ThuVienKyThuatDLPage;
