...
const PopupTimKiemKhachHang = ({ visible, handleClose, setValue }) => {
  ...
  const onSubmit = (data) => {
    const result = Array.from({ length: 10 }, (_, i) => {
      ...
      return {
        MaKhachHang: c,
        TenKhachHang: faker.commerce.productAdjective(),
        checked: false,
      };
    });
  ...
  const handleSave = () => {
    ...
    if (result) {
      setValue("KhachHang", result?.MaKhachHang);
    }
  ...
  return (
    ...
    >
      <form id="search_KhachHang" onSubmit={form.handleSubmit(onSubmit)}>
...
export default PopupTimKiemKhachHang;
